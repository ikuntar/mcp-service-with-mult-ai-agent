...
import { SystemExecutorInstance } from './instance';

export class SystemExecutorFactory implements IExecutorFactory {
  ...
  async create(tool: Tool, token: string): Promise<ExecutorInstance> {
    const rules = await this.ruleManager.getRules(token, 'system');
